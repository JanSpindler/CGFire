 ...
 * ext_spline.h
 *
 * simple cubic ext_spline interpolation library without external
 * dependencies
 *
 * ---------------------------------------------------------------------
...
// ext_spline interpolation
        ...
        public:
            ...
            // boundary condition type for the ext_spline end-points
            enum bd_type {
                first_deriv = 1,
                second_deriv = 2
            };
        ...
        protected:
            ...
            std::vector<double> m_b,m_c,m_d;        // ext_spline coefficients
        ...
        public:
            ...
            // adjust coefficients so that the ext_spline becomes piecewise monotonic
            // where possible
            ...
            // evaluates the ext_spline at point x
...
#ifdef HAVE_SSTREAM
            // ext_spline info string, i.e. ext_spline type, boundary conditions etc.
...
// ext_spline implementation
// -----------------------

        void ext_spline::set_boundary(ext_spline::bd_type left, double left_value,
                                      ext_spline::bd_type right, double right_value)
        {
        ...
        void ext_spline::set_coeffs_from_b()
        {
        ...
        void ext_spline::set_points(const std::vector<double>& x,
                                    const std::vector<double>& y,
                                    spline_type type)
        {
            ...
            if(type==linear) {
                ...
                // boundary conditions
                if(m_left == ext_spline::second_deriv) {
                    // 2*c[0] = f''
                    A(0,0)=2.0;
                    A(0,1)=0.0;
                    rhs[0]=m_left_value;
                } else if(m_left == ext_spline::first_deriv) {
                ...
                if(m_right == ext_spline::second_deriv) {
                    // 2*c[n-1] = f''
                    A(n-1,n-1)=2.0;
                    A(n-1,n-2)=0.0;
                    rhs[n-1]=m_right_value;
                } else if(m_right == ext_spline::first_deriv) {
        ...
        bool ext_spline::make_monotonic()
        {
            assert(m_x.size()==m_y.size());
            assert(m_x.size()==m_b.size());
            assert(m_x.size()>2);
...
// return the closest idx so that m_x[idx] <= x (return 0 if x<m_x[0])
        size_t ext_spline::find_closest(double x) const
        {
        ...
        double ext_spline::operator() (double x) const
        {
        ...
        double ext_spline::deriv(int order, double x) const
        {
            assert(order>0);
            size_t n=m_x.size();
            size_t idx = find_closest(x);
...
#ifdef HAVE_SSTREAM
        std::string ext_spline::info() const
{
    std::stringstream ss;
    ...
    if(m_made_monotonic) {
        ss << "(ext_spline has been adjusted for piece-wise monotonicity)";
    }
    return ss.str();
}
#endif // HAVE_SSTREAM
